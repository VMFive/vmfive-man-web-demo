(function () {
    var target3DGesture = ['TopLeft', 'BottomRight', 'TopRight', 'BottomLeft'];

    addGyroMoment(target3DGesture, 10, 3)



    function addGyroMoment(target3DGesture, fetchFrequency, moveThreshold) {
        // gyro.min.js
        ...
        // var penPosX = 0, penPosY = 0, penPosZ = 0;
        // var pen = getEl("#pen");
        TweenMax.set('.hideFirst', { autoAlpha: 0 })

        var tl = new TimelineMax();

        tl.set('.drawAnimation', { autoAlpha: 0 })
            .set('.motion-end', { autoAlpha: 0 })
            .to('.motion-start', .3, { autoAlpha: 0 })
            .to('.drawAnimation', 0, {
                autoAlpha: 1, onComplete: function () {
                    const myVivus = new Vivus('drawSVG', { duration: 100 });
                }
            })
            .to('.drawAnimation', .3, { autoAlpha: 0 }, '+=1.2')
            .to('.motion-end', .6, { scale: 1, autoAlpha: 1 }).pause();

        // getData 
        gn.init({
            frequency: fetchFrequency,			// ( How often the object sends the values - milliseconds )
            ...
            gn.start(function (data) {
                ...
                // log('#beta_log', 'beta: ' + beta)
                // var oldX = aX;
                // var oldY = aY;
                // var oldZ = aZ;

                // newX = oldX * Math.cos(-degToRad(alpha)) - oldY * Math.sin(-degToRad(alpha));
                // newY = oldY * Math.cos(-degToRad(alpha)) + oldX * Math.sin(-degToRad(alpha));

                //rotate about x axis
                // oldY = newY;
                // newY = oldY * Math.cos(-degToRad(beta)) - oldZ * Math.sin(-degToRad(beta));
                // newZ = oldZ * Math.cos(-degToRad(beta)) + oldY * Math.sin(-degToRad(beta));
                //rotate about y axis
                // oldZ = newZ;
                // oldX = newX;

                // newZ = oldZ * Math.cos(-degToRad(gamma)) - oldX * Math.sin(-degToRad(gamma));
                // newX = oldX * Math.cos(-degToRad(gamma)) + oldZ * Math.sin(-degToRad(gamma));

                // newX = (isNaN(newX) || undefined) ? +0 : newX.toFixed(1);
                // newY = (isNaN(newX) || undefined) ? +0 : newY.toFixed(1);
                // newZ = (isNaN(newX) || undefined) ? +0 : newZ.toFixed(1);


                var tempX = (Math.abs(+aX) < threshold) ? 0 : +aX;
                var tempY = (Math.abs(+aY) < threshold) ? 0 : +aY;

                var angle = fetchAnimation(-tempX, -tempY);

                if (animationArr[animationArr.length - 1] != angle && animationArr[animationArr.length - 2] != angle) {

                    animationArr.push(angle);
                    log(animationArr.join(' , '));
                    if (angle == target3DGesture[matchCount]) {
                        matchCount++;
                        log(angle, matchCount, target3DGesture[matchCount])
                    }

                    if (matchCount >= target3DGesture.length) {
                        log('match');
                        isDrawn = true;
                        drawSvg();
                ...
                if (animationArr.length > 50) {
                    animationArr.shift();
                }
                // vX = (Math.abs(vX)<.5) ? 0 : +(vX * 0.7).toFixed(2);
                // vY = (Math.abs(vY)<.5) ? 0 : +(vY * 0.7).toFixed(2);

                // penPosX += vX;
                // penPosY += vY;

                // pen.style.marginLeft = range(penPosX , 250, -250) + 'px';
                // pen.style.marginTop = range(penPosY, 250, -250) + 'px';

                // timeLine1.append(new Date().getTime(), aX);
                // timeLine2.append(new Date().getTime(), aY);
                // timeLine3.append(new Date().getTime(), aZ);
                // log('#t1_1', 'ax: \t ' + aX);
                // log('#t1_2', 'ay: \t ' + aY);
                // log('#t1_3', 'az: \t ' + aZ);

                // timeLine4.append(new Date().getTime(), newX);
                // timeLine5.append(new Date().getTime(), newY);
                // timeLine6.append(new Date().getTime(), newZ);
                // log('#t2_1', 'new x: \t ' + newX);
                // log('#t2_2', 'new y: \t ' + newY);
                // log('#t2_3', 'new z: \t ' + newZ);

                // timeLine7.append(new Date().getTime(), vX);
                // timeLine8.append(new Date().getTime(), vY);
                // // timeLine9.append(new Date().getTime(), vZ);
                // log('#t3_1', 'gx: \t ' + vX);
                // log('#t3_2', 'gy: \t ' + vY);
                // log('#t3_3', 'gz: \t ' + vZ);
    ...
    function playEnd() {
        ...
        tl.set('.endCut', { scale: 1 })
            .to('.endCut', .6, { scale: 1, autoAlpha: 1, ease: Power2.easeOut })
            .to('.drawBox', 0, { autoAlpha: 0 })
            // .set('.endCut',{scale:.4,autoAlpha:1})

            .to('.mainContainer', .8, { top: 10, ease: Power1.easeInOut })
            .to('.txtWrap .t1', .3, { autoAlpha: 0 }, '-=0.6')
            .to('.txtWrap .t2', .8, { autoAlpha: 1, top: -44 }, '-=0.6')
    }



    function fetchAnimation(x, y) {
        var angle = (y < 0) ? Math.atan(x / y) + Math.PI : Math.atan(x / y);
        angle = (angle < 0) ? angle + 2 * Math.PI : angle;
        angle = (isNaN(angle)) ? 0 : radToDeg(angle);
    ...
    function createTimeline() {
        ...
        drawLine(chart1, timeLine1, 'red', 'chart01');
        drawLine(chart1, timeLine2, 'blue', 'chart01');
        drawLine(chart1, timeLine3, 'green', 'chart01');

        var chart2 = new SmoothieChart();
        drawLine(chart2, timeLine4, 'red', 'chart02');//x
        drawLine(chart2, timeLine5, 'blue', 'chart02');//y
        drawLine(chart2, timeLine6, 'yellow', 'chart02');//nexX

        var chart3 = new SmoothieChart();
        drawLine(chart3, timeLine7, 'red', 'chart03');//x
        drawLine(chart3, timeLine8, 'blue', 'chart03');//y
        drawLine(chart3, timeLine9, 'yellow', 'chart03');//Z
    }

    function drawLine(chart, timeline, storkColor, chartId) {
        chart.addTimeSeries(timeline, { strokeStyle: storkColor, lineWidth: 2 });
        chart.streamTo(document.getElementById(chartId), 200);
    }

    function log(id, string) {
        getEl(id).innerHTML = string;
    }
