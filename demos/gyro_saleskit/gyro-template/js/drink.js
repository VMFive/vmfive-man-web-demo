(function () {
    ...
    var beerWrap = getEl('.liquidContainer');
    var beer = getEl('.liquid');
    ...
    tl.set(getEl('.motion-in-end'),{autoAlpha:0,scale:0.2})
    .to(getEl('.motion-out-start'),0.1,{autoAlpha:0})
    .to(getEl('.motion-in-end'),0.6,{autoAlpha:1,scale:1}).pause();
    ...
    gn.init({
        ...
        gn.start(function (data) {
            ...
            if (beerAngle > 0) {
                temp = - beerAngle * 100 / (startAngle - endAngle);
                waterLine = Math.max(waterLine, temp);
            }
            ...
            TweenMax.to(beer, .3, { top: waterLine+"%" });
