(function () {
    ...
    // var gamma;
    // var alpha;
    var manWrap = getEl('.manContainer');
    var beerWrap = getEl('.beerWrap');
    var beer = getEl('.beer');
    ...
    gn.init({
        ...
        gn.start(function (data) {
            ...
            var { beta, gamma, alpha } = data.do;

            if (!lockTurn) {
                // 不知道為什麼垂直他的alpha就會突然跳到，調整了一個數字發現85是比較接近正確的微調
                if (beta > 85) {
                    alpha -= 360;
                }

                if (alpha > 180) {
                    alpha = limitRange(alpha - 360, -turnAngle, 0);
                } else {
                    alpha = limitRange(alpha, 0, turnAngle);
                }


                if (Math.abs(alpha) >= turnAngle) {
                    lockTurn = true;
                    goCheers(beta);
                }

                alpha = alpha / 40 * 150 - 150;
                // manWrap.style.left = alpha + '%';
                TweenMax.to(manWrap,.1,{left:alpha + '%'})
            }
            ...
            if (isDrink) {
                var rad = Math.atan2(data.dm.gx, -data.dm.gy);
                var deg = limitRange(- rad * (180 / Math.PI), -90, 90);
                var startAngle = 17;
                var endAngle = 90;
                var temp;
                // log(deg.toFixed(0));

                var beerAngle = Math.abs(deg).toFixed(0) - startAngle;

                if (beerAngle > 0) {
                    temp = - beerAngle * 380 / (endAngle - startAngle);
                    waterLine = Math.min(waterLine, temp);
                }
                // // log(beerAngle,temp,waterLine);
                // // beer.style.bottom = waterLine + 'px';
                // // beerWrap.style.transform = "rotate(" + (deg) + "deg)";
                TweenMax.to(beerWrap,.3,{rotation: deg});
                TweenMax.to(beer,.3,{bottom: waterLine});

                // log(deg)
                if(Math.abs(deg).toFixed(0)>=90){
                    isDrink=false;
                    goEnd();
                }
    ...
    function goCheers(betaNow) {
        getEl('.icon').classList.remove('turn');
        getEl('.icon').classList.add('cheers');
        tiltTarget = betaNow - 25;
        isCheers = true;
    }
    // goDrink();
    function goDrink() {
        TweenMax.to('.frontHand', .35, {scale: 0.95, marginBottom: -10, repeat:1,yoyo:true,ease: Power1.easeInOut,onComplete:function(){
            TweenMax.fromTo('.drinkWrap',1,{top:'100%',autoAlpha:1},{top: '0', ease: Power2.easeInOut,onComplete: function () {
                isDrink = true;
            }})
            getEl('.icon').classList.remove('cheers');
            getEl('.icon').classList.add('drink');
        }})
        
    }
    // goEnd();
    function goEnd() { 
        TweenMax.fromTo('.endCut',.6,{delay:0.3,ease:Power4.easeOut,top:'100%',autoAlpha:1},{top:0, onComplete: function () {
        }})
     }
    ...
    setInterval(function () {
        ...
        var bb = addBB();
        TweenLite.fromTo(bb, getRandom(3, 6),
            { css: { left: getRandom(20, 80) + '%', top: '80%', scale: getRandom(.1, .25) } },
            {
                css: { top: getRandom(15, 50) + '%' }, ease: Power1.easeIn, onComplete: function () {
                    removeEl(bb)
                }
            })
